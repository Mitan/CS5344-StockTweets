...
filtered_users_file = 'Formatted_graph.txt'
def main(argv):
		...
		for line in conf_file:	
			...
			for item in node_array[1:]:
				if item in user_dict:
					user_dict[node_array[0].rstrip().lstrip()].append(item)
					count = count + 1
					#print item
			if (count == 0):
				if (len(user_dict[node_array[0].rstrip().lstrip()]) != 0):
					continue
				node_id = node_array[0].rstrip().lstrip()
				filtered_f.write('%s %s'%(node_id,0)+'\n')
			else:
				result_string = ''
				result_string += node_array[0]
				result_string += ' %s'%(count)
				for adjacent_node in  user_dict[node_array[0].rstrip().lstrip()]:
